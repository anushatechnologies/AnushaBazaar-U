...
const mapProducts = (json: any): Product[] => {
    ...
    return (Array.isArray(list) ? list : []).map((p: any) => {
          ...
          const normalizedVariants: Variant[] = variants.map((v: any) => {
                  ...
                  // Backend uses `discountPrice` as the ACTUAL SELLING PRICE (not discount amount).
                                                                   // Use it directly when > 0, otherwise fall back to `price`.
                                                                   const sellingPrice = v.sellingPrice ?? (v.discountPrice != null && v.discountPrice > 0
                                                                                                                   ? v.discountPrice
                                                                                                                   : mrp);
                  return {
                            ...v,
                  ...
                  normalizeImageUrl(
                            p.imageUrl ||
                              ...
                              (Array.isArray(p.imageUrls) ? p.imageUrls[0] : "")
                                                     ...
                                                     // Use backend minPrice as fallback if available (already the lowest selling price)
                                                     const displayPrice = firstVariant
            ? firstVariant.sellingPrice
                                                             : (p.minPrice != null && p.minPrice > 0 ? p.minPrice : p.price ?? 0);
                                                     ...
                                                     return {
                                                             ...
                                                             price: displayPrice,
                                                             originalPrice: firstVariant ? firstVariant.mrp : (p.price ?? 0),
                                                     };
    });
};
