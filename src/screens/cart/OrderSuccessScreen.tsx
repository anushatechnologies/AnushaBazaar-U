...
import { View, Text, StyleSheet, TouchableOpacity, Animated, ScrollView, Image, ActivityIndicator } from "react-native";
...
const OrderSuccessScreen = () => {
  ...
  const handleTrackOrder = () => {
    ...
    setNavigating(true);
    // Replace to prevent going back to success screen
    navigation.replace("OrderTracking", { orderId, orderNumber: route.params?.orderNumber });
  };
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <View style={[styles.footer, { paddingBottom: Math.max(insets.bottom, scale(20)) }]}>
        <Animated.View style={[styles.redirectBox, { opacity: opacityAnim }]}>
           <ActivityIndicator size="small" color="#0A8754" />
           <Text style={styles.redirectText}>Redirecting to tracking in {timeLeft}s...</Text>
        </Animated.View>

        <TouchableOpacity 
<<<<<<< HEAD:src/screens/OrderSuccessScreen.tsx
          style={styles.trackBtn} 
          onPress={() => navigation.navigate("OrderTracking", { orderId, orderNumber })}
=======
          style={[styles.trackBtn, navigating && styles.trackBtnDisabled]} 
          onPress={handleTrackOrder}
          disabled={navigating}
>>>>>>> 8f07c6e (hello):src/screens/cart/OrderSuccessScreen.tsx
        >
          <Text style={styles.trackBtnText}>{navigating ? "Tracking..." : "Track Your Order"}</Text>
        </TouchableOpacity>
