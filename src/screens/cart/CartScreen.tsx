...
const CartScreen = () => {
  ...
  const finalTotal = total + handlingCharge + applicableSmallCartFee;
    ...
    <View style={styles.cartCard}>
      <View style={styles.cardContent}>
        <Image
          source={
            (() => {
              const imgUri = item.image || item.imageUrl || item.productImage || item.thumbnail;
              if (typeof imgUri === "string" && imgUri.length > 0) {
                return { uri: imgUri };
              }
...
<<<<<<< HEAD:src/screens/CartScreen.tsx
...
>>>>>>> 8f07c6e (hello):src/screens/cart/CartScreen.tsx
              </View>
              {appliedCoupon ? (
                <View style={styles.billLine}>
                  <Text style={[styles.billKey, { color: "#0A8754" }]}>Coupon: {appliedCoupon}</Text>
                  <Text style={[styles.billValue, { color: "#0A8754" }]}>-₹{discount.toFixed(2)}</Text>
                </View>
              ) : (
                <View style={styles.billLine}>
                  <Text style={[styles.billKey, { color: "#0A8754" }]}>Total Savings</Text>
                  <Text style={[styles.billValue, { color: "#0A8754" }]}>
...
=======
                <Text style={[styles.billValue, { color: "#0A8754", fontWeight: "800" }]}>FREE</Text>
              </View>
              {handlingCharge > 0 && (
                <View style={styles.billLine}>
                  <Text style={styles.billKey}>Handling Charges</Text>
                  <Text style={styles.billValue}>₹{handlingCharge.toFixed(2)}</Text>
                </View>
              )}
              {applicableSmallCartFee > 0 && (
                <View style={styles.billLine}>
                  <Text style={styles.billKey}>Small Cart Fee</Text>
                  <Text style={styles.billValue}>₹{applicableSmallCartFee.toFixed(2)}</Text>
                </View>
              )}
...
>>>>>>> 8f07c6e (hello):src/screens/cart/CartScreen.tsx
              ...
              >
                <View style={styles.checkoutLabelStack}>
                  <Text style={styles.checkoutPrice}>₹{finalTotal.toFixed(2)}</Text>
