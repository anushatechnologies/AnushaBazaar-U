...
const SERVER_NOTIFS_CACHE_KEY = "@server_notifs_cache";
    ...
    try {
      ...
      if (jwtToken) {
        ...
        if (data !== null) {
          ...
          AsyncStorage.setItem(SERVER_NOTIFS_CACHE_KEY, JSON.stringify(serverNotifs)).catch(() => {});
        } else {
          // If API fails (data is null), load from cache
          try {
            const cached = await AsyncStorage.getItem(SERVER_NOTIFS_CACHE_KEY);
            if (cached) {
              serverNotifs = JSON.parse(cached);
            }
          } catch (e) {}
        }
      }
      ...
      try {
        ...
        if (cached) {
           ...
           const fcmNotifs = localNotifications.map((n: any) => ({ ...n, source: "local" }));
           setNotifs([...fcmNotifs, ...parsed]);
        } else {
