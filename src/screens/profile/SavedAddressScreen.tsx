...
const SavedAddressScreen = () => {
  ...
  const resetForm = () => {
    ...
    setNewPostalCode("");
    setNewTag("home");
    // Reset contact & receiver
    let phone = user?.phone || "";
    if (phone && !phone.startsWith("+91")) phone = `+91${phone}`;
    else if (!phone) phone = "+91";
    setContactName(user?.name || "");
    setContactPhone(phone);
    setOrderingForOther(false);
    setReceiverName("");
    setReceiverPhone("+91");
  };
    ...
    return (
      <View style={[styles.card, item.isDefault && styles.cardDefault]}>
        ...
        {(item.contactName || item.contactPhone) && (
          <Text style={styles.nameText}>{item.contactName}{item.contactPhone ? ` · ${item.contactPhone}` : ''}</Text>
        )}

        <Text style={styles.addressText}>{fullAddress}</Text>

        {item.receiverName ? (
          <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: scale(8), backgroundColor: '#F0FDF4', padding: scale(8), borderRadius: scale(8) }}>
            <Ionicons name="people-outline" size={scale(14)} color="#0A8754" style={{ marginRight: scale(6) }} />
            <Text style={{ fontSize: scale(12), color: '#0A8754', fontWeight: '600' }}>Receiver: {item.receiverName}{item.receiverPhone ? ` · ${item.receiverPhone}` : ''}</Text>
          </View>
        ) : null}
          ...
          <View style={{ flexDirection: "row", gap: scale(10) }}>
            <TouchableOpacity 
              ...
              onPress={() => {
                ...
                setContactName(item.contactName || user?.name || "");
                let ph = item.contactPhone || user?.phone || "";
                if (ph && !ph.startsWith("+91")) ph = `+91${ph}`;
                setContactPhone(ph || "+91");
                if (item.receiverName) {
                  setOrderingForOther(true);
                  setReceiverName(item.receiverName || "");
                  setReceiverPhone(item.receiverPhone || "+91");
                } else {
                  setOrderingForOther(false);
                  setReceiverName("");
                  setReceiverPhone("+91");
  ...
  const handleSaveBtn = async () => {
    ...
    if (orderingForOther) {
      ...
      if (receiverPhone.length < 13) {
        Alert.alert("Invalid Phone", "Please enter a valid 10-digit receiver phone number.");
        return;
      }
    }
    ...
    try {
      ...
      if (orderingForOther && receiverName.trim()) {
        payload.receiverName = receiverName.trim();
        payload.receiverPhone = receiverPhone.trim();
      }
      ...
      >
        ...
        >
          ...
          >
             ...
             <View style={styles.formGroup}>
               ...
               <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                 ...
                 <TextInput 
                   ...
                   onChangeText={(t) => {
                     if (!t.startsWith("+91")) {
                       setContactPhone("+91" + t.replace(/^\+?9?1?/, ""));
                     } else {
                       setContactPhone(t);
                     }
                   }}
             ...
             <Pressable
               style={[styles.receiverToggle, orderingForOther && styles.receiverToggleActive]}
             ...
             >
               <View style={{ flexDirection: 'row', alignItems: 'center', flex: 1 }}>
                 ...
                 <View style={{ marginLeft: scale(12), flex: 1 }}>
                   <Text style={{ fontSize: scale(14), fontWeight: '700', color: '#111827' }}>Ordering for someone else?</Text>
                   <Text style={{ fontSize: scale(12), color: '#6B7280', marginTop: scale(2) }}>Add receiver's details for delivery</Text>
                 </View>
               </View>
               <Ionicons
             ...
             {orderingForOther && (
               <View style={{ backgroundColor: '#F0FDF4', borderRadius: scale(12), padding: scale(14), marginBottom: scale(20), borderWidth: 1, borderColor: '#D1FAE5' }}>
