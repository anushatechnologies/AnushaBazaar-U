  ...
  const fetchOrders = useCallback(async () => {
    ...
    try {
      ...
      const sorted = ordersArray.sort((a, b) => {
        const timeA = new Date(a.placedAt || a.createdAt || a.orderDate || a.date || 0).getTime();
        const timeB = new Date(b.placedAt || b.createdAt || b.orderDate || b.date || 0).getTime();
        return timeB - timeA;
      });
      setOrders([...sorted]);
    } catch (error: any) {
  ...
  const handleReorder = (items: any[]) => {
    ...
    items.forEach(item => {
      ...
      const cartItem = {
        ...
        image: item.imageUrl || item.productImage || item.image || item.thumbnail || "",
        unit: item.unit || "",
      };
      addToCart(cartItem);
    });
